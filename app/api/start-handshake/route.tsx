...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(
    `https://www.spot.im/api/sso/v1/register-user?code_a=${body.code_a}&access_token=${process.env.NEXT_PUBLIC_OPENWEB_SSO_TOKEN}&primary_key=${tempUser.id}&user_name=${tempUser.username}&email=${tempUser.email}&email_verified=true&display_name=${tempUser.display_name}`,
    {
      cache: "no-store",
    }
  );
